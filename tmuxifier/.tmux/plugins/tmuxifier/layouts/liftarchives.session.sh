...
session_root "$ACTUAL_HOME/project/perso/liftarchives"

# Create session with specified name if it does not already exist. If no
# argument is given, session name will be based on layout file name.
if initialize_session "stuvy"; then
  new_window "code"
  run_cmd "cd $session_root && clear && nvim ."
  new_window "claude"
  run_cmd "cd $session_root && claude"
  new_window "server"
  run_cmd "cd $session_root && bun run dev:worker"
  new_window "git"
